...
export const MediaCard: React.FC<MediaCardProps> = ({ 
  ...
  if (layout === 'horizontal') {
      return (
        <div className={`group relative rounded-[24px] overflow-hidden flex cursor-pointer backdrop-blur-lg border transition-all duration-300 min-h-[120px] ${
            isDark 
            ...
            : 'bg-white/60 border-white/50 hover:shadow-xl hover:bg-white/80'
             ...
             <div className="pr-4 flex items-center justify-center">
                 <div className={`w-8 h-8 rounded-full border flex items-center justify-center transition-all ${
                     isDark ? 'border-white/10 text-white group-hover:bg-white group-hover:text-black' : 'border-gray-200/50 text-gray-400 group-hover:bg-[#0b5cff] group-hover:text-white group-hover:border-[#0b5cff]'
                 }`}>
  ...
  return (
    <div className={`group rounded-3xl overflow-hidden flex flex-col h-full relative backdrop-blur-xl transition-all duration-300 ${
      isDark 
      ...
      <div className="relative h-64 overflow-hidden">
        ...
        <div className="absolute top-4 left-4 z-20">
          <span className={`px-4 py-1.5 text-xs font-bold tracking-wide rounded-full shadow-lg backdrop-blur-md border ${
            isDark ? 'bg-black/40 text-white border-white/20' : 'bg-white/80 text-[#00053d] border-white/50'
          }`}>
            {place.category}
          </span>
        </div>
      ...
      <div className={`p-6 flex flex-col flex-1 ${isDark ? 'border-t-0 rounded-b-3xl' : ''}`}>
        ...
        <h3 className={`font-heading font-bold text-2xl mb-3 leading-tight transition-colors ${
           isDark ? 'text-white group-hover:text-[#4ab5ff]' : 'text-[#00031f] group-hover:text-[#0b5cff]'
        }`}>
          {place.title}
        </h3>
        ...
        <div className={`mt-auto pt-5 flex items-center justify-between ${
           ...
           <button className={`text-sm font-bold flex items-center gap-1.5 transition-all group-hover:gap-2.5 ${
               isDark ? 'text-white' : 'text-[#0b5cff]'
           }`}>
